...
class PreprocessUtilsTest(tf.test.TestCase):
    ...
    def testNoFlipWhenProbIsZero(self):
        ...
        with self.test_session():
            actual, is_flipped = utils.flip_dim([image], prob=0, dim=0)
            self.assertAllEqual(numpy_image, actual.eval())
            self.assertAllEqual(False, is_flipped.eval())
            actual, is_flipped = utils.flip_dim([image], prob=0, dim=1)
            self.assertAllEqual(numpy_image, actual.eval())
            self.assertAllEqual(False, is_flipped.eval())
            actual, is_flipped = utils.flip_dim([image], prob=0, dim=2)
            self.assertAllEqual(numpy_image, actual.eval())
            self.assertAllEqual(False, is_flipped.eval())
    ...
    def testFlipWhenProbIsOne(self):
        ...
        with self.test_session():
            actual, is_flipped = utils.flip_dim([image], prob=1, dim=0)
            self.assertAllEqual(dim0_flipped, actual.eval())
            self.assertAllEqual(True, is_flipped.eval())
            actual, is_flipped = utils.flip_dim([image], prob=1, dim=1)
            self.assertAllEqual(dim1_flipped, actual.eval())
            self.assertAllEqual(True, is_flipped.eval())
            actual, is_flipped = utils.flip_dim([image], prob=1, dim=2)
    ...
    def testFlipMultipleImagesConsistentlyWhenProbIsOne(self):
        ...
        with self.test_session() as sess:
            image, label, is_flipped = utils.flip_dim(
    ...
    def testReturnRandomFlipsOnMultipleEvals(self):
        ...
        with self.test_session() as sess:
            actual, is_flipped = utils.flip_dim(
    ...
    def testReturnCorrectCropOfSingleImage(self):
        ...
        [cropped] = utils.random_crop([image_placeholder],
                                      crop_height,
                                      crop_width)
    ...
    def testReturnConsistenCropsOfImagesInTheList(self):
        ...
        [cropped_image, cropped_label] = utils.random_crop(
    ...
    def testDieOnRandomCropWhenImagesWithDifferentWidth(self):
        crop_height, crop_width = 2, 3
        image1 = tf.placeholder(tf.float32, name='image1',
                                shape=(None, None, 3))
        image2 = tf.placeholder(tf.float32, name='image2',
                                shape=(None, None, 1))
        cropped = utils.random_crop(
    ...
    def testResizeTensorsToRange(self):
        ...
        for i, test_shape in enumerate(test_shapes):
            ...
            new_tensor_list = utils.resize_to_range(
                image=image,
                label=None,
                min_size=min_size,
                max_size=max_size,
    ...
    def testResizeTensorsToRangeWithFactor(self):
        ...
        for i, test_shape in enumerate(test_shapes):
            ...
            with self.test_session() as session:
                ...
                self.assertEqual(new_tensor_list[0].shape,
                                 expected_image_shape_list[i])
                self.assertEqual(new_tensor_list[1].shape,
                                 expected_label_shape_list[i])
